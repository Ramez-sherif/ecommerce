...
class OrdersService {
  ...
  static Future createOrderFromCart(CartModel cart) async {
    ...
    for (var cartProduct in cartProducts.entries) {
      await addOrder(
          cartProduct.key.id, userId, cartProduct.value, orderId);
  ...
  static Future addOrder(String productId, String userId, int quantity,
  ...
  static Future<List<OrdersModel>> getAllOrders(String userId,UserModel user) async {
    ...
    await db
      ...
      for (var doc in value.docs) {
        orderProducts[allProducts.firstWhere(
            (element) => element.id == doc["product_id"])] = doc["quantity"];
      }
    });
